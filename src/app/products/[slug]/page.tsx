...
export default async function ProductDetailPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="font-sans grid grid-rows-[20px_1fr_20px] justify-items-center min-h-screen p-8 pb-20">
      <main className="flex flex-col gap-6 row-start-2 items-start w-full max-w-4xl">
        <Link href="/">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full bg-[color:var(--surface)]/70 border border-[color:var(--surface-border)]/60 p-4 rounded-lg">
          <div className="aspect-square rounded overflow-hidden bg-[color:var(--surface)]/30">
            {image ? (
              ...
              <div className="w-full h-full grid place-items-center text-xs text-[color:var(--muted)]">No image</div>
            )}
          </div>
          <div className="flex flex-col gap-3">
            <h1 className="text-xl font-semibold">{product.title}</h1>
            <div className="text-lg font-medium text-[color:var(--foreground)]">{product.price.toFixed(2)}</div>
            <div className="text-xs inline-flex items-center gap-2 text-[color:var(--muted)]">
              <span className="rounded bg-[color:var(--surface)]/70 border border-[color:var(--surface-border)]/60 px-2 py-1">{product.category?.name || "Uncategorized"}</span>
            </div>
